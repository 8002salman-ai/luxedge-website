...
## Phase 3B — FINAL LIVE VERIFICATION (2026-08-17, migration applied + real product E2E)

- **0004 applied:** owner ran `0004_reconcile_live.sql` in the Dashboard SQL Editor. Verified live: all **30 V2 tables exist** (customers, suppliers, supplier_products, supplier_variants, supplier_shipping, reviews, ai_providers, agent_jobs, agent_runs, agent_logs, product_candidates, product_scores, creative_assets, creative_jobs, campaigns, ad_creatives, ad_performance, + others) — MISSING: NONE. 5 legacy-only tables remain untouched (35 total).
- **RLS / grants (live probes):** anon reads categories/products/reviews → 200 filtered; anon reads suppliers/ai_providers/customers/agent_jobs/campaigns → 200 EMPTY (RLS filters, no rows); anon POST products/customers/suppliers → **42501 blocked**; buyer POST suppliers → 403; admin reads all customers + writes suppliers → allowed; service role reads → allowed. RLS enabled and enforced on products/customers (proved by 42501 on writes).
- **Customer sync (live):** created a real non-admin buyer (Auth Admin API, `email_confirm:true` — normal signup is email rate-limited in this project), signed in with password → **no admin role claim**; `customers` row created via the user's OWN JWT (ensureCustomerProfile path) with matching `auth_user_id`; buyer sees ONLY their own row; buyer cannot read admin tables (empty) nor write them (403). Browser: sign in → session survives refresh → logout works. Test buyer + customers row deleted afterward (cleanup).
- **One real product E2E (PASS):** **KONG Classic, Durable Natural Rubber Dog Toy** — verified source https://www.kongcompany.com/kong-classic/ (manufacturer page fetched; 100% natural rubber, made in USA, SKU/UPC table), retail reference Chewy Medium $11.96. Seeded via service role (admin path): category Pet Toys, supplier KONG Company, supplier_product with evidence, product_candidate (approved), **product published** ($11.96, slug `kong-classic-dog-toy`, inventory ledger 50), 4 real images (all HTTP-200 CDN URLs). Storefront catalog **source = SUPABASE** — homepage/shop render ONLY the KONG product (demo products gone from catalog; the 2 demo names seen on-screen are the persisted localStorage cart, not catalog). PDP verified: title, $11.96, Pet Toys, image gallery, In Stock, **"No verified reviews yet"** (no fabricated ratings), Description/Specifications/Reviews(0) tabs. Admin → Products table shows KONG / Pet Toys / $11.96 / 50. Cart: added, qty+ → subtotal +$11.96, remove → back to prior 2 items. No paid order placed.
- **NEW migration 0005 (`0005_drop_legacy_currency_checks.sql`) — owner action:** the live legacy schema kept `products_currency_check` (accepts only lowercase 'usd'); V2 defaults to 'USD', so every V2 write fails with 23514 until it is dropped. 0005 drops every `%_currency_check`/`%_country_check` constraint in public (additive, idempotent, transactional). The seeded product currently stores `currency='usd'`; once 0005 is applied it can be normalized to 'USD' by any V2 update. **Please paste 0005 into the Dashboard SQL Editor and run it.**
- **Other legacy gaps noted (not blocking this E2E):** legacy `products.title` is NOT NULL (V2 writes `name`) — seeders must set both; legacy `product_images.storage_path`/`public_url` NOT NULL (V2 writes `url`) — set both; legacy `addresses.street1` etc. NOT NULL — V2 address writes need both column sets (Phase 3C work); `orders.order_number` is an identity column. No demo data was bulk-seeded.
- **Quality gate:** `npm test` ✅ 88/88 · `npx tsc --noEmit` ✅ 0 errors · `npm run build` ✅ (storefront 410.45 KB / gzip 116.97 KB, admin lazy 242.37 KB). Zero console errors across storefront + admin.
