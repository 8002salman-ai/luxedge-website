...
interface ProductRow { id: string; slug?: string | null; status?: string | null; supplier_source?: string | null; cost_price?: number | null; us_inventory?: boolean | null; stock_status?: string | null; inventory_qty?: number | null; commerce_readiness?: string | null; }
interface CategoryRow { slug: string; }
interface BlogRow { slug: string; }
const num = (v: unknown): number => { const n = Number(v); return Number.isFinite(n) ? n : 0; };
function commerceReady(p: ProductRow): boolean {
  if (typeof p.commerce_readiness === 'string' && p.commerce_readiness) return p.commerce_readiness === 'COMMERCE_READY';
  ...
  return num(p.cost_price) > 0 && (p.us_inventory === true || (p.stock_status === 'in_stock' && num(p.inventory_qty) > 0));
}
