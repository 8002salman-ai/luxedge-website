...
export const TERMS_SECTIONS: PolicySection[] = [
  ...
  { title: 'Governing Law', body: 'Luxedge is operated by Embani LLC, registered in the United States. The state whose law governs these Terms has not been confirmed by the operator, so this section deliberately names no jurisdiction and no venue. Until it does, any dispute should first be raised with us at hello@luxedge.us so we can try to resolve it directly. This page will be updated with the governing state once the operator supplies it.' },
...
export const RETURNS_SECTIONS: PolicySection[] = [
  { title: 'Our Promise', body: 'If you receive a product that is damaged, defective, or incorrect, contact us within 30 days of your order date and we will work to resolve it as quickly as we can. This policy covers orders that arrive wrong or faulty; it is not a change-of-mind return programme.' },
  { title: 'Return Eligibility', body: 'Return requests must be made within 30 days of the original order date. Products must be unused, unopened, and returned in their original packaging. Returns need prior approval from Luxedge before they are shipped back, so please contact us first.' },
