// ============================================================================
// LUXEDGE — GET /api/admin/media-stats
//
// First-party per-video view counts for the Media Manager listing page (Views
// column). Mirrors /api/admin/blog-stats exactly: one request, tiny response —
// the aggregation runs server-side via the service role so the browser never
// downloads raw analytics rows (no N+1, no per-visit Supabase egress).
//
// Data is REAL site_events analytics only:
//   views    — count of `page_view` events whose path matches /media/<slug>
...
// /media/my-video and /media/my-video/ count as one page.
//
// Window: last 90 days, capped at 50,000 events per pass (same cap as the
// Traffic dashboard). Videos with no events simply have no entry (the UI shows
// "—").
//
...
// In-worker TTL cache: the media manager polls this on every mount; a 5-minute
// cache keeps repeat visits free of any Supabase round-trip.
let cachedStats: { at: number; data: Record<string, MediaStats> } | null = null;
...
export default async function handler(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  try {
    ...
    // page_view rows carry the video path. Media paths only — the like filter
    // keeps the download small while the trailing-normalization still handles
    // sub-paths (/media/my-video and /media/my-video/ both land on the video).
    for (let page = 0; page < MAX_PAGES; page++) {
      ...
      for (const row of rows) {
        ...
        if (!m) continue; // /media hub itself, etc.
        const slug = m[1];
